...
- **propagation_required：**
  ...
  - 事务的加入是由**调用方的事务决定的**，如果调用方已经存在一个事务，那么被调用的方法会加入这个已经存在的事务。
...
- 私有方法上使用@Transactional注解通常不会生效，因为**Spring的事务管理是基于代理的**，而私有方法无法被代理拦截。

###### 同一个类中方法互调

- 当一个@Transactional标注的方法在**同一个类中被另一个方法直接调用时**，事务也不会生效。Sping 的代理方式导致的；

###### 异常类型不触发回滚

- 默认情况下，**只有运行时异常和错误才会触发事务回滚**。如果你抛出的是受检异常（即非运行时异常），则不会触发事务回滚。
  - throw new Exception("这是一个受检异常")； 不会回滚；
  - 针对这种异常，可以定制 rollbackFor 属性，支持回滚；
