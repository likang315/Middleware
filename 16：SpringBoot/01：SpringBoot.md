...
- Spring Boot 是一个用于**简化基于Spring的应用程序开发的框架**，它通过提供默认配置和约定俗成的设置，使得开发者可以更快速地搭建和部署 Spring 应用程序。
- Spring Boot 可以把Web应用程序变为**可自执行的 Jar 文件**，不用部署到传统 Java 应用服务器里就能在**命令行里运行**。Spring Boot 在应用程序里内嵌了一个Servlet 容器（Tomcat、Jetty或Undertow），以此实现这一功能。但这是**内嵌的Servlet容器提供的功能**，不是Spring Boot实现的。
...
- 在 pom.xml 中引入 spring-boot-dependencies, 它可以提供 dependency management，引入以后在申明其它dependency的时候就不需要version了，可以用于解决以来冲突；
...
##### 04：安装 SpringBoot CLI

- 下载分发包：
- 添加系统路径：在解压后的目录里，你会找到一个bin 目录，其中包含了一个spring.bat 脚本（用于Windows环境）和一个**spring 脚本**（用于Unix环境）。 把这个bin目录添加到系统路径里。
- $ spring --version

##### 05：搭建 SpringBoot 项目【重要】
...
- 复合注解 **@SpringBootApplication**
   ...
   - @EnableAutoConfiguration【自动配置】
     ...
     - 这个 spring.factories 文件也是一组一组的**key=value的形式**，其中一个key是EnableAutoConfiguration类的全类名，而它的value**是一个xxxxAutoConfiguration的类名的列表**，这些类名以逗号分隔；

     - Spring启动的时候会扫描所有 jar 路径下的`META-INF/spring.factories`，将其文件包装成**Properties对象**，从Properties对象获取到key值为`EnableAutoConfiguration`的数据，然后通过`@Conditional`按需加载的配置类，添加到 IOC 容器里边。

   - Springboot 会自动初始化数据源，若没有配置数据源则剔除 DataSourceAutoConfiguration 该类；
...
- **传统的 War 包部署（需要打包部署到 tomcat 的webapp下）**。但这里的 main() 方法让你可以**在命令行里把该应用程序当作一个可执行 Jar 文件来运行**。
...
- SpringBootTest：该注解从 AtlantisZeusApplicationTests 配置类例加载Spring应用程序上下文。若不想启动加载整个 Spring，只需要指定某个类即可；
...
- 构建插件的主要功能是：把项目打包成一个可执行的 Jar 包，包括把应用程序的所有依赖打入 Jar 文件内，并为 Jar 添加一个描述文件，其中的内容能让你用 **java -jar** 来运行应用程序。
...
- Spring Boot的自动配置**是应用程序启动时的过程，利用条件化 Bean，才决定 Spring 配置实例化哪个。**
- 在向应用程序加入Spring Boot时，有个名为**spring-boot-autoconfigure 的JAR文件**，其中包含了 很多配置类。**每个配置类都在应用程序的Classpath里**，都有机会实例化其 Bean。
...
1. Spring-Boot 默认提供静态资源目录位置**需置于classpath下**（resources目录），目录名需符合如下规则：
...
3. 全局异常处理（AOP）
   ...
   ```java
   /**
   * 定义一个全局异常处理器，只要Controller发生异常就会调到这里
   */
   @ControllerAdvice
...
4. 渲染 Web 页面
   ...
   1.  模板引擎：基于模板来生成文本输出，实现动态的网页，相当于Jsp；
   2. 模板引擎
      - Thymeleaf
      - FreeMarker
      - Velocity
...
5. **项目启动成功**
   <img src="https://github.com/likang315/Middleware/blob/master/16%EF%BC%9ASpringBoot/photos/springboot.png?raw=true" style="zoom:40%;" />
