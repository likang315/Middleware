...
一致性非锁定读(consistent nonblocking read)
	指InnoDB 通过多行版本控制的方式来读取当前执行时间数据中的数据.如果当前数据正在执行写操作,这时读取操作不会因此阻塞
	而是去读取⼀个快照数据(副本)，快照数据就是当前行数据的历史版本
...
8：锁造成的问题
	...
	3:不可重复读：不可重复读本质上读不加锁带来的，因此会让同一事务的两次读操作取回不一致的结果,解决办法是读完再写
	4:幻象读：幻象读本质上是写的时候未加锁，解决办法是读完再写
...
9：死锁检测(dead lock)
   ...
   解决死锁: 
	...
	2：Wait-for-graph 主动检测:通过“等待获取的锁” 和 “等待获取该锁的事务”, 构造出⼀张有向图如果图中存在回路, 
				   就代表存在死锁, 一旦发现回路, 就将其中一个回滚, 另⼀个事务就得以继续执行 被回滚的事务
				   会返回"dead lock"
...
10：事务：指的是满足 ACID 特性的任意一种操作
	...
	事务的隔离性由锁来实现, 原子性, 子致性和和 Undo Log 来完成，持久性由 Redo Log 实现
...
11：事务实现
  ...
  2：Undo Log（回滚日志）记录了事务的行为, 可以很好的对页进行 "重做" 操作, 但事务有时候需要进行回滚, 
	...
	重做日志的主要工作是将数据库逻辑地恢复到原来的样子, 但数据结构和页本身在回滚之后可能和事务开始前不太相同, 因为与
	此同时有大量的并发事务存在, 不能简单的将一个页回滚到事务开始时的样子, 否则会影响其他事务，恢复行记录

	Undo Log 的另一个功能是 MVCC,当需要读取的记录已经被其他事务加锁的时候, 当前事务可以通过 undo 读取之前的版本, 以此
	实现一致性非锁定读
