...
### 1：Object 类：Java 中 的顶级父类

###### Object 类是所有 Java 中类的最高父类

普通类继承了 Object 类才可以称为一个Java类

在这个类中提供了最基本的**9个方法（加上重载是11个方法**）
...
Object 提供了 toString() 用来默认的解析对象的内部结构，当前的这个默认解析是Object类给我们的一种实现，需重写成我们想要解析的格式
...
**一致性（consistent）**：对于任意不为null的引用值x和y，如果用于equals比较的对象信息没有被修改的话，多次调用时x.equals(y) 要么一致地返回true 要么一致地返回 false

###### 3：hashCode(): 哈希表（散列）

对象调用该方法时，返回的是一个根据其在内存中的物理地址而有关的整数，在一个程序中没有改变对象中封装的数据，则对象多次调用该方法时产生同一个整数（哈希码）
...
1：如果两个对象的 equals（）相等，那么它们的hashCode（）一定要相等。也告诉我们重写equals方法，一定要重写hashCode方法
...
2：重写的clone() 必须实现 Cloneable 接口。虽然这个接口并没有什么方法，但是必须实现该标志接口,如果不实现将会在运行期间，抛出：CloneNotSupportedException 异常
...
### 3：Scanner 类：扫描器，从指定源开始扫描，等待键盘输入，创建对象新建扫描器,开始扫描
...
1：一定要读取到有效字符后才可以开始输入，在未读取到有效字符时，空格，tab,Enter键均舍去
...
###### java.lang.Math（无构造）在这个类中提供大量的关于数学运算的 static 方法	
...
返回一个伪随机数，它是取自此随机数生成器序列的、在 0（包括）和指定值（不包括）之间均匀分布的 int 值左闭右开
...
### Class SimpleDateFormat：是一个以与语言环境有关的方式来格式化和解析日期的具体类
...
### ResourceBundle 类：用于加载本地化资源文件的方便类

##### 当程序需要一个特定于语言环境的资源时（如 String），程序可以从适合当前用户语言环境的资源包中加载它

主要依靠 java.util 包中的 **ResourceBundle类与Local 类**
...
```java
//注意：一定要将 properties 文件创建在src目录下

//以特定语言环境输出，创建Local对象进行语言环境封装
