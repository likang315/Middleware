...
**full gc** 是通过 vm thread 执行的，单线程的，整个过程是 stop-the-world，在该过程中会判断当前 gc 是否需要进行compact（压缩），即把存活对象移动到内存的一端，可以有效的消除cms gc产生的碎片空间

###### 触发 full gc 的主要原因是：在eden区为对象或TLAB分配内存失败，导致一次 YGC
