...
Integer.toString() 将i转换为 buf 符号数组，然后new String(buf, true)													String.valueof()方法
...
String：适用于**少量的字符串操作的情况，线程安全**（final修饰）
...
调用 intern（）方法，会检查常量池中是否有和当前的对象字面量相同的引用对象，若有，则返回字符串池中的对象，若没有则放到字符串池中，并返回当前对象
