...
- `update` API 遵循同样的规则， 从外部来看，我们在一个文档的某个位置进行部分更新。然而在内部， `update` API 与之前描述相同的检索-修改-重建索引的处理过程；
...
- 对于部分更新的很多使用场景，文档已经被改变也没有关系，可以通过 设置参数 `retry_on_conflict` 来自动完成， 这个参数规定了失败之前 `update` 应该重试的次数，它的默认值为 `0` 。
...
- 这种格式类似一个有效的单行 JSON 文档 *流* ，每行一定要以换行符(`\n`)结尾， *包括最后一行* 。这些换行符被用作一个标记，可以有效分隔行；
...
- **多大是太大了？**
  - 整个批量请求都需要由接收到请求的节点加载到内存中，因此该请求越大，其他请求所能获得的内存就越少。 批量请求的大小有**一个最佳值**，大于这个值，性能将不再提升，甚至会下降。 但是最佳值不是一个固定的值。它完全取决于硬件、文档的大小和复杂度、索引和搜索的负载的整体情况。
