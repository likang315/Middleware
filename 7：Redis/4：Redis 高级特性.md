...
**SAVE ,BGSAVE 生成RDB 文件，区别在在于**：前者会阻塞服务器进程，知道RDB文件创建完毕为止，在服务器京城阻塞期间，服务器是不可以处理任何任务命令请求的，而后者不会，后者会fork一个子进程，取创建RDB文件
...
5：RDB每一条数据只有一条记录，不会向AOF一条数据可能有多条记录，所以每条数据只需要写一次就够了，因此文件载入快
