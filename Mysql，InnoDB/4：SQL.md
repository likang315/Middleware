...
投影(projector)：部分列组成的新的集合 SELECT name as emp_name,sex,age FROM emp; 选择(selector)：部分行组成新的集合 ，分表存储 SELECT * FROM stu WHERE id<10;
...
2：关联查询要添加连接条件，否则会产生**笛卡尔积** 它是一个无意义的结果集，它的记录数是与所有参与查询的表的记录数乘积的结果，当数据量大时，会出现内存溢出
