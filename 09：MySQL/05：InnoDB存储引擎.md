...
1. 负责刷新缓存池中的数据，保证缓存池中的缓存的是最新的数据；
2. 将已经修改的数据文件刷新到磁盘文件（刷盘），同时保证数据库在发生异常的情况下 InnoDB 能恢复到正常的运行状态；

###### 后台线程

1. **Master Thread：**将缓存池中的数据**异步刷新到磁盘**，保证数据一致性。
   - 脏页的刷新，合并插入缓冲（Inset Buffer）；

2. **IO Thread：**InnoDB 使用了大量的 AIO 处理写请求，该线程负责处理这些 IO 请求的回调处理；
3. **Purge Thread：**用于事务提交后，**回收 Undo Log**；
4. **Page Clear Thread：**用于刷新脏页到缓存中，释放 Master Thread 的压力；

##### 02：内存

###### 缓冲池（innodb_buffer_pool）

- InnoDB 存储引擎是基于磁盘存储的，并将其中所有的记录**按照页**的方式进行管理，由于 CPU 和磁盘速度的鸿沟，采用**缓存池技术**来提高数据库的性能，一块很大的内存区域；
- 在 DB 中进行读取页的时候，首先将从磁盘读到的页存放到缓存池中, 该过程称为将页"FIX"在缓冲池中。下次再读相同页的时候先判断是否在缓冲池中，若在缓冲池中，命中缓存，直接读取该页；
- 对于写操作，**首先修改缓冲池中的页，然后再以一定的频率刷新到磁盘上**。刷盘操作并不是每次发生页修改时触发，而是通过**"CheckPoint 机制"**，刷新回磁盘；
- 缓冲池的大小默认为 **256KB**，innodb_buffer_pool_size：设置大小；

###### LRU 列表 & FLUSH 列表

- midpoint insertion strategy
  - InnoDB 使用 LRU 算法管理缓冲池，并且在 LRU 列表中加入了 midpoint 位置，新读取的页并不是直接放入到 LRU 列表的头部，而是放入到 midpoint 位置。**防止批量查询可能导致热点页被刷新出缓冲池**；
  - 默认在 LRU 列表的 5/8 处，可以手动配置。
  - 在 InnoDB中，把 midpoint 之后的列表称为 Old 列表，之前的列表称为 New 列表。**New 列表都是最为活跃的热点数据；**
- 压缩页
  - InnoDB 存储引擎从1.x 之后开始支持压缩页的功能，将 16K 的页压缩至1KB、2KB、4KB、8KB；同时使用 **unzip_LRU列表管理压缩页；**
- FLUSH 列表
  - 用于**管理脏页的数据结构**。这个列表跟踪了需要被写入磁盘的脏页，以便后续的后台线程将其刷新至磁盘，确保数据持久化。

###### 重做日志缓冲（redo log buffer）

- InnoDB 首先将 Redo Log 信息放入这个缓冲区，然后以一定的频率刷新到磁盘的 Redo Log 文件中。不需要设置过大，一般会 1s 刷新一次，只需要保证 1s 内产生的事务在这个缓冲大小之内即可；
- innodb_log_buffer_size，默认 8M；
...
- 缩短宕机数据库的恢复时间；
- 缓冲池不够用时，将脏页刷盘；
- redo log buffer 不可用时，刷新脏页；

###### 两种 CheckPoint 

- Sharp CheckPoint：发生数据库关闭时，所有脏页全部刷新回磁盘上；
- Fuzzy CheckPoint：只刷新一部分脏页，不是全部刷新；

##### 04：Master Thread 的工作

###### Loop

- redo log buffer 刷新到磁盘上，即使这个事务还没有被提交；（1/s）
- 合并插入缓冲（可能）；(1/s)
- 回收无用的  undo log 页；（1/10s ）

##### 05：InnoDB 关键特性

###### 插入缓冲（Insert Buffer）

- 在 InnoDB 中，主键是唯一的标识，在**插入记录的顺序是按照主键递增的顺进行**，因此不需要磁盘的随机读取。在⼀般情况下，不需要随机读取另⼀个页中的记录，因此 InnoDB 如果采用**自增主键的方式**去插入速度会非常快。
- 但是，更多情况下表中除了主键以外还存在其他索引，在进行插入操作时，数据页的存放依然是按照主键顺序存放，但是对于**非聚簇索引的叶子节点的插入就不再是顺序的，此时就需要离散的访问非聚簇索引页，从而产生大量随机读取导致插入性能下降**。
- 对于非聚簇索引的插入或者更新，不是一次直接插入到索引页中，而是**先判断插入的非聚簇索引页是否在缓冲池中，如果在，直接插入，否则先放入⼀个 Insert Buffer 对象**，作为占位符，然后再以⼀定的频率进行 Insert Buffer 和 索引叶子节点的 merge 操作，通过将多个插入操作合并到一个操作中（因为在一个索引页中），提高插入效率；
...
- 为了提高查询效率，索引其实是就一个B+树，索引段和数据段都存储在硬盘上，**读取一个节点一次磁盘 I/O 操作**。
