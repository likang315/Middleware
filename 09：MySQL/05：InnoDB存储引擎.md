...
- Dynamic：从 MySQL5.7 版本之后，默认使用 Dynamic 行格式；也是紧凑的行格式；
- Compressed 
...
- 当数据表的字段都定义成 NOT NULL 的时候，这时候表里的行格式就不会有 NULL 值列表，至少占用 1 Byte空间；
...
- 联合索引的规则
  ...
  - | 联合索引                                                     | 索引字段是否用到 |
    ...
    | select * from stu where a >= xx and b=xx； (包含等会就会用到) | a 和 b 都用到    |
