...
b})}};
AJS.$(document).ready(function(){function c(b){var a=!1;b.each(function(){var b=AJS.$("input[name='"+this.name.replace("checkbox","initial")+"']");if(this.checked&&0===b.length||!this.checked&&"on"===b.val())return a=!0,!1});return a}var a=AJS.$("#select-options");AJS.$(".perms-dropdown-trigger").click(function(){var b=AJS.$(this);a.data("entity-type",b.data("entity-type")).data("entity-id",b.data("entity-id"))});a.find("a").click(function(b){var c="";AJS.$(this).hasClass("select-all")&&(c="checked");
AJS.Confluence.SpacePermissions.setPermissionsState(a.data("entity-type"),a.data("entity-id"),c);b.preventDefault()});(-1!==window.location.href.indexOf("/spaces/editspacepermissions.action")||-1!==window.location.href.indexOf("/spaces/doeditspacepermissions.action?key="))&&AJS.$('input[type="submit"]').click(function(){if("cancel"!==AJS.$(this).attr("name")){AJS.$(this).prop("disabled",!0);var b="usersToAddButton"===AJS.$(this).attr("name"),a="groupsToAddButton"===AJS.$(this).attr("name"),a=a||c(AJS.$('table#gPermissionsTable input[type="checkbox"]')),
b=b||c(AJS.$('table#uPermissionsTable input[type="checkbox"]'));c(AJS.$('table#aPermissionsTable input[type="checkbox"]'))&&AJS.trigger("analyticsEvent",{name:"confluence.space.permission.change.anonymous",data:{}});a&&AJS.trigger("analyticsEvent",{name:"confluence.space.permission.change.group",data:{}});b&&AJS.trigger("analyticsEvent",{name:"confluence.space.permission.change.user",data:{}});AJS.$('form[name="editspacepermissions"]').submit()}})});
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
;
;try {
/* module-key = 'com.mute.confluence.plugins.xTables:xtables.plugin-resources', location = '/js/jquery.initialize.js' */
(function(d){var c=function(e,f){this.selector=e;this.callback=f};var b=[];b.initialize=function(e,g){var f=[];callbackOnce=function(){if(f.indexOf(this)==-1){f.push(this);d(this).each(g)}};d(e).each(callbackOnce);this.push(new c(e,callbackOnce))};var a=new MutationObserver(function(e){for(var f=0;f<b.length;f++){d(b[f].selector).each(b[f].callback)}});a.observe(document.documentElement,{childList:true,subtree:true,attributes:true});d.fn.initialize=function(e){b.initialize(this.selector,e)}})(jQuery);
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
...
;
;try {
/* module-key = 'com.mute.confluence.plugins.xTables:xtables.plugin-resources', location = '/js/xtables.plugin.js' */
AJS.$(document).ready(function(){var a={};a.buttonActions=AJS.$();a.buttonFilters=AJS.$();a.wysiwygIframe=AJS.$();a.confluenceTables=[];a.lastScrollT=0;a.lastScrollL=0;a.storedFilters=[];a.filterCell=AJS.$();a.currentTd=AJS.$();a.xify=function(){if(!a.buttonActions.length){AJS.$("#page").append('<div id="x-controls" class="x-buttons"><div class="x-more"></div><div class="x-options"><div id="x-calc" class="x-container"><div id="x-direction" class="aui-buttons x-o-container"><button id="x-o-v" class="x-v aui-button aui-button-primary"><span class="aui-icon aui-icon-small aui-icon-v"></span></button><button id="x-o-h" class="x-h aui-button"><span class="aui-icon aui-icon-small aui-icon-h"></span></button></div><div  id="x-operation" class="aui-buttons x-o-container"><button id="x-o-sum" class="aui-button"><span class="aui-icon aui-icon-small aui-icon-sum"></span></button><button id="x-o-prod" class="aui-button"><span class="aui-icon aui-icon-small aui-icon-prod"></span></button><button id="x-o-av"  class="aui-button x-adve x-morable"><span class="aui-icon aui-icon-small aui-icon-av"></span></button><button id="x-o-min"  class="aui-button x-adve x-morable"><span class="aui-icon aui-icon-small aui-icon-min"></span></button><button id="x-o-max"  class="aui-button x-adve x-morable"><span class="aui-icon aui-icon-small aui-icon-max"></span></button><button id="x-o-om"  class="aui-button x-advc"><span class="aui-icon aui-icon-small aui-icon-adv"></span></button></div></div><div class="x-tip"></div></div></div>');a.buttonActions=AJS.$("#x-controls.x-buttons")}a.wysiwygIframe=AJS.$("#wysiwyg iframe");a.buttonFilters.hide();a.buttonActions.find(".x-more").click(function(){a.showXControls(a.buttonActions,true);AJS.$(document).keyup(function(d){if(d.keyCode==27){a.buttonActions.hide()}})});a.buttonActions.find(".aui-button.x-advc").click(function(d){d.preventDefault();AJS.$(this).parent().find(".x-adve").removeClass("x-adve-hidden");AJS.$(this).hide()});var c='<style type="text/css">.x-sum{border:2px dashed #b4d3d8}.x-prod{border:2px dashed #d0e0b3}.x-av{border:2px dashed #e0b3b3}.x-min{border:2px dashed #e0c9b3}.x-max{border:2px dashed #bbb3e0}</style>';a.wysiwygIframe.contents().find("head").append(c);a.buttonsScrollUpdate(AJS.$(a.wysiwygIframe[0].contentWindow),a.buttonActions);AJS.$(".toolbar-contextual").initialize(function(){var d=setInterval(function(){a.positionXButtons(a.currentTd,a.buttonActions)},20);setTimeout(function(){clearInterval(d)},1000)});a.confluenceTables=a.wysiwygIframe.contents().find(".confluenceTable");a.updateXTables();tinymce.get("wysiwygTextarea").onChange.add(function(){a.updateXTables()});tinymce.get("wysiwygTextarea").onKeyUp.add(function(d,f){if(AJS.$.isNumeric(f.key)||[",","."," "].indexOf(f.key)>-1||[8,46].indexOf(f.keyCode)>-1){a.updateXTables()}});tinymce.get("wysiwygTextarea").onNodeChange.add(function(h,k,j){a.confluenceTables=[];a.currentTd=AJS.$();var g=AJS.$(j);if(g.closest("table")){var m=AJS.$(g.closest("table"));var d=a.getSelectionType(m);if(d){a.confluenceTables[0]=m;var l=AJS.$();if(g.closest("td").length||g.nodeName=="td"){l=AJS.$(g.closest("td"))}if(g.closest("th").length||g.nodeName=="th"){l=AJS.$(g.closest("th"))}if(l.length){var i=(l.attr("colspan")>1||l.attr("rowspan")>1);if(i){a.resetCell(l);a.buttonActions.hide()}else{a.currentTd=l;a.showXControls(a.buttonActions,false);a.buttonActions.show();a.buttonActions.find(".x-o-container").each(function(){var e=AJS.$(this);if(e.find(".aui-button.x-adve").length){e.find(".aui-button.x-advc").show();e.find(".aui-button.x-adve").addClass("x-adve-hidden")}else{e.find(".aui-button.x-advc").hide()}});a.positionXButtons(l,a.buttonActions);var n="";var f="";if(d=="v"){a.buttonActions.find(".x-h").addClass("x-vh-hidden").removeClass("x-vh-visible");a.buttonActions.find(".x-v").addClass("x-vh-visible").removeClass("x-vh-hidden");n="v"}else{if(d=="h"){a.buttonActions.find(".x-h").addClass("x-vh-visible").removeClass("x-vh-hidden");a.buttonActions.find(".x-v").addClass("x-vh-hidden").removeClass("x-vh-visible");n="h"}else{a.buttonActions.find(".x-v").removeClass("x-vh-hidden").removeClass("x-vh-visible");a.buttonActions.find(".x-h").removeClass("x-vh-hidden").removeClass("x-vh-visible")}}if(l.hasClass("x-op")){AJS.$.each(l.attr("class").split(/\s+/),function(e,o){if(a.getOpType(o)!=""){f=a.getOpType(o)}if(a.getOpDirection(o)!=""){n=a.getOpDirection(o)}})}a.fixGroupButtonsClasses(n,f);a.buttonActions.find("#x-direction button").each(function(){if(!AJS.$(this).hasClass("x-advc")){AJS.$(this).unbind("click").click(function(e){e.preventDefault();n=AJS.$(this).attr("id").split("-")[2];a.applyXCell(l,n,f,m)})}});a.buttonActions.find("#x-operation button").each(function(){if(!AJS.$(this).hasClass("x-advc")){AJS.$(this).unbind("click").click(function(e){e.preventDefault();if(AJS.$(this).hasClass("aui-button-primary")){f=""}else{f=AJS.$(this).attr("id").split("-")[2]}a.applyXCell(l,n,f,m)})}})}}else{a.buttonActions.hide()}}else{a.buttonActions.hide()}}else{a.buttonActions.hide()}})};a.updateXTables=function(){if(a.confluenceTables.length>0){AJS.$.each(a.confluenceTables,function(c,d){a.addTableIndexes(AJS.$(d));a.updateXCalc(AJS.$(d))})}};a.addTableIndexes=function(c){var d=0;var e=[];c.find("tbody").children().each(function(h,f){var i=AJS.$(f);a.removeClassContaining(i,"xtr-");i.addClass("xtr-"+d);var g=0;i.children().each(function(j,q){var k=AJS.$(q);if(k.hasClass("x-op")){if(!k.text()){a.resetCell(k)}}a.removeClassContaining(k,"xtd-");var m="xtd-"+d+"-"+g;while(e.indexOf(m)>-1){g++;m="xtd-"+d+"-"+g}k.addClass(m);var n=k.attr("rowspan");var o=function(){if(n>0){for(var r=1;r<n;r++){var s=d+r;m="xtd-"+s+"-"+g;k.addClass(m);e.push(m)}}};o();var p=k.attr("colspan");if(p>0){for(var l=1;l<p;l++){g++;k.addClass("xtd-"+d+"-"+g);o()}}g++});d++})};a.buttonsScrollUpdate=function(c,d){c.scroll(function(){var j=c.scrollTop();var h=c.scrollLeft();var g=j-a.lastScrollT;var f=h-a.lastScrollL;if(g!=0){var i=d.position().top-g;d.css({top:i+"px"});a.lastScrollT=j}if(f!=0){var e=d.position().left-f;d.css({left:e+"px"});a.lastScrollL=h}})};a.positionXButtons=function(h,g){var f=true;if(g.attr("id")=="x-controls"){if(g.find(".x-options").is(":visible")){f=false}}if(f){var i=0;if(g.attr("id")=="x-controls"){if(AJS.$("#wysiwyg").length){i=AJS.$("#wysiwyg")[0].getBoundingClientRect().top}}var d=h[0].getBoundingClientRect();var c=d.top+i+10-30-16+AJS.$(window).scrollTop();var e=d.right-10+AJS.$(window).scrollLeft();g.css({top:c+"px",left:e+"px"})}};a.fixGroupButtonsClasses=function(d,c){a.buttonActions.find(".aui-button").removeClass("aui-button-primary");a.fixButtonsClasses(d);a.fixButtonsClasses(c)};a.fixButtonsClasses=function(d){var c=a.buttonActions.find("#x-o-"+d);if(c.hasClass("x-adve")){c.removeClass("x-adve").removeClass("x-adve-hidden")}c.addClass("aui-button-primary")};a.showXControls=function(e,d){if(d){e.find(".x-options").show();e.find(".x-more").hide();var g=e.position().top;var c=e.position().left;if(e.attr("id")=="x-controls"){g-=10;c-=123}else{g+=4;c-=123}var f=115;if(c<0){f=115+c;c=0}e.find(".x-tip").css({"background-position":f+"px 0"});e.css({top:g+"px",left:c+"px"})}else{e.find(".x-options").hide();e.find(".x-more").show()}};a.removeClassContaining=function(c,d){if(c.attr("class")){AJS.$.each(c.attr("class").split(/\s+/),function(e,f){if(f.indexOf(d)>-1){c.removeClass(f)}})}};a.resetCell=function(c){c.removeClass("x-v");c.removeClass("x-h");c.removeClass("x-m");c.removeClass("x-vt");c.removeClass("x-vb");c.removeClass("x-hl");c.removeClass("x-hr");c.removeClass("x-sum");c.removeClass("x-prod");c.removeClass("x-av");c.removeClass("x-min");c.removeClass("x-max");c.removeClass("x-op")};a.applyXCell=function(c,f,e,d){console.log("Here applyXCell");console.log(e);a.fixGroupButtonsClasses(f,e);if(e!=""&&f!=""){if(d.find("tr .mceSelected").length>0){d.find("tr .mceSelected").each(function(){var g=AJS.$(this);a.resetCell(g);g.addClass("x-op x-"+e+" x-"+f)})}else{a.resetCell(c);c.addClass("x-op x-"+e+" x-"+f)}a.updateXCalc(d)}else{a.resetCell(c)}};a.getSelectionType=function(e){if(e.find("tr .mceSelected").length>0){var h=false;var g=false;var f=-1;var i=-1;var c;var d;e.find("tr .mceSelected").each(function(){AJS.$.each(AJS.$(this).attr("class").split(/\s+/),function(j,k){if(k.indexOf("xtd")>=0){c=parseInt(k.split("-")[1]);d=parseInt(k.split("-")[2]);if(f==-1){f=c}else{if(f!=c){h=true}}if(i==-1){i=d}else{if(i!=d){g=true}}}})});if((g||h)&&!(g&&h)){if(g){return"v"}else{return"h"}}else{return false}}else{return true}};a.getOpType=function(c){switch(c){case"x-sum":return"sum";break;case"x-prod":return"prod";break;case"x-av":return"av";break;case"x-min":return"min";break;case"x-max":return"max";break;default:return""}};a.getOpDirection=function(c){switch(c){case"x-v":return"v";break;case"x-h":return"h";break;case"x-m":return"m";break;case"x-vt":return"vt";break;case"x-vb":return"vb";break;case"x-hl":return"hl";break;case"x-hr":return"hr";break;default:return""}};a.updateXCalc=function(c){c.find(".x-op").each(function(){var h;var o;var g;var e;var n;AJS.$.each(AJS.$(this).attr("class").split(/\s+/),function(q,r){if(a.getOpType(r)!=""){h=a.getOpType(r)}if(a.getOpDirection(r)!=""){o=a.getOpDirection(r)}if(r.indexOf("xtd")>=0){n=r;e=parseInt(r.split("-")[1]);g=parseInt(r.split("-")[2])}});var j=0;var p=0;var k=0;var m=1;var f=null;var l=0;var i=false;AJS.$(this).closest(".confluenceTable tbody").children().each(function(){AJS.$(this).children().each(function(){var t=false;var r=AJS.$(this);var q=r.text();var s=(q.indexOf(" ")>-1);q=r.text().replace(/\s/g,"").replace(",",".");AJS.$.each(r.attr("class").split(/\s+/),function(w,x){if(x.indexOf("xtd")>-1){var u=parseInt(x.split("-")[1]);var v=parseInt(x.split("-")[2]);if(!(x==n)){if(AJS.$.isNumeric(q)){if((o==="v")&&v==g){t=true;return false}else{if((o==="h")&&u==e){t=true;return false}}}}}});if(t){if(s){i=true}l=parseFloat(q);switch(h){case"sum":k+=l;break;case"prod":m*=l;break;case"av":k+=l;break;case"min":if(l<f||f===null){f=l}break;case"max":if(l>f||f===null){f=l}break;default:}j++}})});if(j===0){p=0}else{switch(h){case"sum":p=k;break;case"prod":p=m;break;case"av":p=k/j;break;case"min":p=f;break;case"max":p=f;break;default:}}var d=p.toFixed(2).replace(/[.,]00$/,"");if(i){String.prototype.reverse=function(){return this.split("").reverse().join("")};d=d.split("").reverse().join("").replace(/((?:\d{2})\d)/g,"$1 ").reverse()}AJS.$(this).text(d)})};AJS.$("#wysiwyg iframe").initialize(function(){AJS.$(this).load(function(){a.xify()})});a.addFilters=function(d,c){d.find("tbody").children().children().each(function(f,j){var h=AJS.$(j);isRawSpan=false;if(!isRawSpan){var e;var g;var i;if(h.attr("class")){AJS.$.each(h.attr("class").split(/\s+/),function(k,l){if(l.indexOf("xtd")>=0&&parseInt(l.split("-")[1])==0){e=parseInt(l.split("-")[1]);g=parseInt(l.split("-")[2]);i=l;return false}})}if(e==0){h.mousemove(function(){a.onCell=true;if(!a.onFilters&&!a.isSearching){a.filterCell=h;a.showXControls(a.buttonFilters,false);a.buttonFilters.show();a.positionXButtons(h,a.buttonFilters);var l=(h.attr("colspan")>1)?h.attr("colspan"):1;var k=a.buttonFilters.find("#x-show-hide-button");var m=a.buttonFilters.find("#x-search-input");var n=(k.hasClass("aui-button-primary"))?true:false;k.unbind("click").click(function(o){o.preventDefault();n=(k.hasClass("aui-button-primary"))?true:false;if(n){d.find(".x-filtered-tr").addClass("x-hidden-tr");k.removeClass("aui-button-primary");a.positionXButtons(h,a.buttonFilters);a.showXControls(a.buttonFilters,true)}else{d.find(".x-filtered-tr").removeClass("x-hidden-tr");k.addClass("aui-button-primary");a.positionXButtons(h,a.buttonFilters);a.showXControls(a.buttonFilters,true)}n=!n;m.focus()});m.unbind("keyup").keyup(function(){h.attr("data-x-filter",AJS.$(this).val());if(AJS.$(this).val().length>0){h.addClass("x-filter-active")}else{h.removeClass("x-filter-active")}for(var o=0;o<l;o++){a.storedFilters[c][g+o]=AJS.$(this).val();a.filterTable(h,c,e,n)}a.positionXButtons(h,a.buttonFilters);a.showXControls(a.buttonFilters,true)})}});h.mouseout(function(){a.onCell=false;setTimeout(function(){if(!a.onFilters&&!a.onCell&&!a.isSearching){a.buttonFilters.hide()}},100)})}}})};a.filterTable=function(f,d,c,h){if(c==0){var g=f.closest("table").find("tbody").first();var e=[];g.children().each(function(i,j){console.log("Line: "+i);AJS.$(j).removeClass("x-filtered-tr");AJS.$(j).removeClass("x-hidden-tr");AJS.$(j).children().each(function(k,l){AJS.$.each(AJS.$(l).attr("class").split(/\s+/),function(o,p){var q=parseInt(p.split("-")[1]);if(p.indexOf("xtd")>-1&&q>0){var r=parseInt(p.split("-")[2]);var n=AJS.$(l).text();var m="";if(a.storedFilters[d][r]){m=a.storedFilters[d][r];if(!(n.toLowerCase().indexOf(m.toLowerCase())>-1)){AJS.$(l).addClass("x-filtered-td");e.push(q)}if(n.toLowerCase().indexOf(m.toLowerCase())>-1){}}}})})});$.each(e,function(k,j){g.children().each(function(i,l){if(AJS.$(l).hasClass("xtr-"+j)){AJS.$(l).addClass("x-filtered-tr");if(!h){AJS.$(l).addClass("x-hidden-tr")}return false}})})}};if(AJS.$("#content.page.view").length){var b=AJS.$("#content.page.view");if(b.find("table").length){a.onCell=false;a.onFilters=false;a.isSearching=false;if(!a.buttonFilters.length){AJS.$("#page").append('<div id="x-filters" class="x-buttons"><div class="x-more"></div><div class="x-options"><div id="x-search" class="x-container"><input class="text" type="text" id="x-search-input" name="x-search-input"><button class="aui-button aui-button-primary" id="x-show-hide-button"><span class="aui-icon aui-icon-small aui-iconfont-view">View</span></button></div><div class="x-tip"></div></div></div>');a.buttonFilters=AJS.$("#x-filters.x-buttons")}a.buttonActions.hide();a.buttonFilters.find(".x-more").click(function(){a.showXControls(a.buttonFilters,true);var c="";var d=a.buttonFilters.find("#x-search-input");if(a.filterCell.length&&a.filterCell.attr("data-x-filter")){c=a.filterCell.attr("data-x-filter")}d.val(c);d.select();d.focus(function(){a.isSearching=true;AJS.$(document).keyup(function(f){if(f.keyCode==27){a.isSearching=false;a.buttonFilters.hide()}})}).focusout(function(){setTimeout(function(){if(!d.is(":focus")){a.isSearching=false;a.buttonFilters.hide()}},150)});d.focus()});a.buttonFilters.hover(function(){a.onFilters=true},function(){a.onFilters=false;setTimeout(function(){if(!a.onCell&&!a.onFilters&&!a.isSearching){a.buttonFilters.hide()}},500)});b.find("table").each(function(d){var c=AJS.$(this);if(!c.find("tr").first().hasClass("xtr-0")){a.addTableIndexes(c)}a.storedFilters[d]=[];a.addFilters(c,d)})}}});
} catch (err) {
    if (console && console.log && console.error) {
        console.log("Error running batched script.");
        console.error(err);
